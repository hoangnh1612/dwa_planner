...
void RRTSTAR::initilaize(cv::Mat map, Pose2D start, Pose2D goal, BGR color, double radius)
{
	this->map = map;
 ...
 * @return
 */
Node* RRTSTAR::neareast(Pose2D point)
{
...
void RRTSTAR::implementAlgorithm()
{
	for (int i = 0; i < this->max_iter; i++)
	{
		...
		if (q)
		{
			Node* qNearest = this->neareast(q->pose);
...
bool RRTSTAR::checkCollision(Point2D start, Point2D end)
{
	...
	for (double i = 0; i <= distance; i+= 0.1)
	{
		...
		Point2DPixel point_pixel = convertMeterToPixel(point, this->gain_x, this->gain_y, this->map_height_pixel);
